...
L9H65["const serialize = require('node-serialize');serialize.unserialize(payload);"]==>L10H65["case0160 "]
...
L9H66["import * as ns from 'xxx'ns.unserialize() import {unserialize} from 'node-serialize'unserialize(taint)import {unserialize as ns} from 'node-serialize'ns(taint)"]==>L10H66["case0161,case0162,case0163 "]
...
L5H84["function Person(){    this.getName = function(){        return taint ;    }}Person.prototype.aaa = function () {    return taint;}let p = new Person();sink(p.aaa());"]==>L6H84["case0179"]
...
L6H94["let res=(a: number,b: number): number=>a+b;let y: number = res;"]==>L7H94["case0187 "]
...
L5H101["function add(a: number, b: number): number;function add(a: string, b: string): string;function add(a: string, b: number): string;function add(a: number, b: string): string;function add(a: Combinable, b: Combinable) {  if (typeof a === 'string' || typeof b === 'string') {    return a.toString() + b.toString();  }  return a + b;}"]==>L6H101["case0194"]
...
L5H102["//a.tsnamespace Test {     export interface a {         do(param);     }}//b.ts/// <reference path = 'a.ts' /> namespace Test {     export class TestA implements a {         public do(param) {             return sink(param);         }      }}//c.ts/// <reference path = 'a.ts' /> /// <reference path = 'b.ts' /> function foo(test: Test.a, x: string){    test.do(x)}foo(new Test.TestA(),taint);"]==>L6H102["case0195"]
...
L6H108["单层简单对象部分属性为污点"]==>L7H108["case025A、case025B、case025AA、case025BB、case025AAA、case025BBB、case025AAAA、case025AAAAA、case025BBBBB "]
...
L7H112["不将字符串算作数组是由于Javascript基础数据类型中String类型为基本数据类型，而数组算引用数据类型，两者本就不同。"]==>L8H112["case0210AB"]
...
L5H118["逻辑求解...不继续细化"]==>L6H118["var abc=ctx.requestvar url='http://baidu.com'if(test){    url=abc}if(!test){    ctx.curl(url)}"]
L6H118["var abc=ctx.requestvar url='http://baidu.com'if(test){    url=abc}if(!test){    ctx.curl(url)}"]==>L7H118["var abc=ctx.requestvar url='http://baidu.com'if(test){     url=abc    test=false}if(!test){    ctx.curl(url)}"]
L7H118["var abc=ctx.requestvar url='http://baidu.com'if(test){     url=abc    test=false}if(!test){    ctx.curl(url)}"]==>L8H118["case0217B、case0218B"]

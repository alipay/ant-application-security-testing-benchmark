...
L4H21["污点链路的完整度"]==>L5H200["污点传播跟踪能力"]
L5H200["污点传播跟踪能力"]===>L5H27["AST节点枚举传播场景"]
...
L5H200["污点传播跟踪能力"]===>L5H201["特殊链路跟踪能力"]
...
M3H1["数据流不可达"]==>M4H5["string a=taintstring a='123'sink(a)"]
M3H1["数据流不可达"]==>M4H6["string b=0string a=taintstring a=bsink(a)"]
M4H6["string b=0string a=taintstring a=bsink(a)"]==>M5H6["b的定义又会有很多场景，不进行展开"]
M3H1["数据流不可达"]==>M4H7["string[] a = [ '1', '2', '3']sink(a[taint])"]
M1H1["准确度"]==>M2H8["对象敏感"]
M2H8["对象敏感"]==>M3H8["不同对象相同属性"]
M1H1["准确度"]==>M2H9["路径敏感"]
M2H9["路径敏感"]==>M3H9["不涉及求解问题"]
M3H9["不涉及求解问题"]==>M4H9["多个return语句"]
M3H9["不涉及求解问题"]==>M4H10["{    ...    return 0;    return sink(taint);}"]
M2H9["路径敏感"]==>M3H11["涉及求解问题"]
M3H11["涉及求解问题"]==>M4H11["逻辑求解"]
M4H11["逻辑求解"]==>M5H11["...不继续细化"]
M3H11["涉及求解问题"]==>M4H12["表达式求解"]
M4H12["表达式求解"]==>M5H12["...不继续细化"]
M4H12["表达式求解"]==>M5H13["string abc=requeststring url='http://baidu.com'if(1+1==2){    url=abc    sinkHTTP(url)}if(1+1>2){    sinkHTTP(url)}"]
M3H11["涉及求解问题"]==>M4H14["string abc=requeststring url='http://baidu.com'if(test){    url=abc}if(!test){    sinkHTTP(url)}"]
M4H14["string abc=requeststring url='http://baidu.com'if(test){    url=abc}if(!test){    sinkHTTP(url)}"]==>M5H14["string abc=requeststring url='http://baidu.com'if(test){     url=abc    test=false}if(!test){    sinkHTTP(url)}"]
M1H1["准确度"]==>M2H15["域敏感"]
M2H15["域敏感"]==>M3H15["属性/元素级别"]
M3H15["属性/元素级别"]==>M4H15["数组/Map/集合元素"]
M4H15["数组/Map/集合元素"]==>M5H15["单维数组/Map/集合中部分元素为污点"]
M4H15["数组/Map/集合元素"]==>M5H16["多维数组/Map/集合中部分元素为污点"]
M4H15["数组/Map/集合元素"]==>M5H17["传递元素不确定"]
M4H15["数组/Map/集合元素"]==>M5H18["sink(a[1])"]
M4H15["数组/Map/集合元素"]==>M5H19["sink(a[0][1])"]
M4H15["数组/Map/集合元素"]==>M5H20["sink(a[b+c])"]
M3H15["属性/元素级别"]==>M4H21["对象属性"]
M3H15["属性/元素级别"]==>M4H22["obj.propertyA"]
M2H15["域敏感"]==>M3H23["字符串中字符"]
M3H23["字符串中字符"]==>M4H23["字符串中部分字符为污点（使用整个字符串）"]

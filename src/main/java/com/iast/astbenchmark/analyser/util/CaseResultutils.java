...
public class CaseResultutils {
    ...
    public static List<CaseResultbean> caseAnalyse(Map<String, BaseOriginalDataBean> tagMap){
        /**
         * 将获取的结果与预期结果进行比较,得到这个Case是否成功
         *  TODO 此处还可以考虑存储原始数据和解析数据 为展示详情打基础
         */
        ...
        caseTargetBeanMap.forEach((k,targetBean)->{
            ...
            resultbean.setCaseTypeEnum(StrUtil.isEmpty(targetBean.getCaseType())?null: CaseTypeEnum.valueOf(targetBean.getCaseType()));
    ...
    private static CaseResultEnum analysisResultEnumList(List<CaseResultEnum> resultEnums) {
         ...
         * FN>FP>>TP>TN
         */
        Map<String,CaseResultEnum> resultEnumMap  = resultEnums.stream().collect(Collectors.toMap(e->e.getCode(), e->e,(k1,k2)->k1));
    ...
    /** 检查是否和预期值一样**/
    private static CaseResultEnum findAndCheckItem(CaseTargetItemBean itemBean,Map<String, BaseOriginalDataBean> tagMap){

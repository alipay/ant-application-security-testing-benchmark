...
// compose = CaseChange_01.java
// bind_url = /sqli/BS00119
// assession information end


@Controller
@RequestMapping(value="/sqli")
public class CaseChange_01 extends HttpServlet {

    private static final Logger log = LoggerFactory.getLogger("CaseChange_01");


    @ResponseBody
    @RequestMapping(value="/BS00119",method = {RequestMethod.POST, RequestMethod.GET})
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        response.setContentType("text/html;charset=UTF-8");
        String param = request.getParameter("BS00119");
        if (param == null){
            param = "";
        }
        if(!param.equals(param.toUpperCase())){
            response.getWriter().write("不是全部大写");
        }

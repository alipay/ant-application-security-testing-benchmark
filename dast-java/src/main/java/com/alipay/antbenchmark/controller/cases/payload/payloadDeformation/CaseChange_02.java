...
// assession project = payload->payload变形->大小写
// compose = CaseChange_02.java
// bind_url = /sqli/BS00120
// assession information end


@Controller
@RequestMapping(value = "/sqli")
public class CaseChange_02 extends HttpServlet {

    private static final Logger log = LoggerFactory.getLogger("CaseChange_02");


    @ResponseBody
    @RequestMapping(value = "/BS00120", method = {RequestMethod.POST, RequestMethod.GET})
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        response.setContentType("text/html;charset=UTF-8");
        String param = request.getParameter("BS00120");
        if (param == null) {
            param = "";
        }
        if (!param.equals(param.toLowerCase())) {
            response.getWriter().write("不是全部小写");
        }

...
// compose = PayloadReplace_04.java
// bind_url = /sqli/BS00150
// assession information end


@Controller
@RequestMapping(value = "/sqli")
public class PayloadReplace_04 extends HttpServlet {
    private static final long serialVersionUID = 1L;


    @ResponseBody
    @RequestMapping(value = "/BS00150", method = {RequestMethod.POST, RequestMethod.GET})
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        response.setContentType("text/html;charset=UTF-8");
        String[] white_list = {"username", "password", "userid"};

        String param = request.getParameter("BS00150");
        if (param == null) {
            param = "";
        }
        int count = 0;
        for (int i = 0; i < white_list.length; i++) {
            if (param.equalsIgnoreCase(white_list[i])) {
                count += 1;
            }
        }
        if (count == 0) {

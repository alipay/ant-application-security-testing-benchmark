...
public class FormatResultUtil {
    ...
    public static String compareAndForamt(CaseDataCollectResultBean resultBean1,
        CaseDataCollectResultBean resultBean2) {

        StringBuffer buffer = new StringBuffer();
        buffer.append("\n");
        buffer.append("对比结果汇总：-----------------------------------------------------------------\n");
        buffer.append(resultBean1.getVendor().getDescription() + "(a):"
            + totalResult(resultBean1.getCaseDetectionItems()) + "\n");
        buffer.append(resultBean2.getVendor().getDescription() + "(b):"
            + totalResult(resultBean2.getCaseDetectionItems()) + "\n");
        buffer.append("差异详情：-----------------------------------------------------------------\n");
    ...
    private static String compare(CaseDataCollectResultBean resultBean1, CaseDataCollectResultBean resultBean2) {
        ...
        for (CaseResultbean caseDetectionItem : resultBean1.getCaseDetectionItems()) {
            String compareres = compareBean(caseDetectionItem, resultbeanMap2.get(caseDetectionItem.getCaseNo()),
                resultBean1.getVendor().getDescription(), resultBean2.getVendor().getDescription());
    ...
    private static String compareBean(CaseResultbean bean1, CaseResultbean bean2, String vendor1, String vendor2) {
        if (bean2 == null) {
            return CaseStuctCache.getLeafByCaseNo(bean1.getCaseNo()).getFullName() + "(" + vendor1 + "(a):"
                + bean1.getResult().getDesc() + "," + vendor2 + "(b):" + "NULL" + ")\n";
        }
        if (!bean1.getResult().equals(bean2.getResult())) {
            return CaseStuctCache.getLeafByCaseNo(bean1.getCaseNo()).getFullName() + "(" + vendor1 + "(a):"
                + bean1.getResult().getDesc() + "," + vendor2 + "(b):" + bean2.getResult().getDesc() + ")\n";
        }
        return null;
    }

    private static Map<String, CaseResultbean> flatResultToMap(CaseDataCollectResultBean resultBean) {
        return resultBean.getCaseDetectionItems().stream()
    ...
    private static String detailItem(List<CaseResultbean> list) {
        ...
        for (CaseTypeEnum value : CaseTypeEnum.values()) {
            ...
            // }). 去掉排序
            List<CaseResultbean> resultEnumList =
                list.stream().filter(e -> value.equals(e.getCaseTypeEnum())).collect(Collectors.toList());
    ...
    private static String formatCaseResult(List<CaseResultbean> resultEnumList) {
        ...
        for (CaseResultbean resultbean : resultEnumList) {
            ...
            buffer.append(
                resultbean.getCaseNo() + "->" + CaseStuctCache.getLeafByCaseNo(resultbean.getCaseNo()).getFullName()
    ...
    private static String detail(List<CaseResultbean> list) {
        ...
        for (CaseTypeEnum value : CaseTypeEnum.values()) {
            buffer.append(value.getDesc() + "\n");
            Map<CaseResultEnum, Long> res = list.stream().filter(e -> value.equals(e.getCaseTypeEnum()))
                .collect(Collectors.groupingBy(CaseResultbean::getResult, Collectors.counting()));
    ...
    private static String totalResult(List<CaseResultbean> list) {
        ...
        Map<CaseResultEnum, Long> res =
            list.stream().collect(Collectors.groupingBy(CaseResultbean::getResult, Collectors.counting()));

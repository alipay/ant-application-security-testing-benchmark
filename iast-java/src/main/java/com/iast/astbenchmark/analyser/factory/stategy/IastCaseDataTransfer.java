 ...
 */
@Component
public class IastCaseDataTransfer implements CaseDataTransfer {
@Autowired
private ConfigService configService;
    ...
    public CaseDataCollectResultBean doOperation() {
        ...
        resultBean.setReportId("IAST@"+time);
       ...
       List<IastSlsBaseMessageBean> logBeans = getReportLog(configService.getIastReportId(),configService.getIastDetectionPath());
         ...
         */
        Map<String, BaseOriginalDataBean> tagMap = convertToTagMap(logBeans);
    ...
    private Map<String, BaseOriginalDataBean> convertToTagMap(List<IastSlsBaseMessageBean> logBeans){
        return logBeans.stream().filter(e->e.getDetail()!=null&&StrUtil.isNotEmpty(e.getDetail().getSinkStack())&&e.getType().equals("taint")).collect(Collectors.toMap(e1->getTagKey(e1),e2->e2,(k1,k2)->k1));
    }
